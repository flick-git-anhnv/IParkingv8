...
namespace Kztek.Tool
{
    public class UdpTools
    {
        ...
        public static async Task<string> ReceiveSocketResponseAsync(string comport, int baudrate, string cmd, bool isRetry)
        {
            string response = await Task.Run(() =>
                {
                    return UdpTools.ExecuteCommand_UTF8(comport, baudrate, cmd, 500);
                });
            if (string.IsNullOrEmpty(response) && isRetry)
            {
                await Task.Delay(200);
                return await ReceiveSocketResponseAsync(comport, baudrate, cmd, false);
            }
