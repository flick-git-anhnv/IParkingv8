...
{
    public partial class FrmReportAlarm : Form, KzITranslate
    {
        #region Properties
        ...
        #region Forms
        public FrmReportAlarm(Image? defaultImage, IAPIServer ApiServer, IEnumerable<Lane> lanes, bool isAllowSelect = false)
        {
            InitializeComponent();
            ...
            this.Load += FrmReportAlarm_Load;
            this.KeyDown += FrmReportAlarm_KeyDown;
            this.FormClosing += FrmReportAlarm_FormClosing;
        }
        private async void FrmReportAlarm_Load(object? sender, EventArgs e)
        {
            ...
            cbAlarmMode.SelectedIndexChanged += ChangeSearchConditionEvent;
        }
        private void FrmReportAlarm_KeyDown(object? sender, KeyEventArgs e)
        {
        ...
        private void FrmReportAlarm_FormClosing(object? sender, FormClosingEventArgs e)
        {
        ...
        private async void DgvData_SelectionChanged(object? sender, EventArgs e)
        {
            try
            {
                ...
                var eventData = await this.ApiServer.ReportingService.Alarm.GetByIdAsync(eventId);
                ...
                if (!string.IsNullOrEmpty(overviewImageData?.PresignedUrl))
                {
                    picOverviewImageIn.ShowImage(await ImageHelper.GetImageFromUrlAsync(overviewImageData?.PresignedUrl) ?? defaultImg);
                }
                else if (!string.IsNullOrEmpty(overviewImageData?.Base64))
                {
                    picOverviewImageIn.ShowImage(ImageHelper.Base64ToImage(overviewImageData?.Base64 ?? "") ?? defaultImg);
                }
                else
                {
                ...
                else if (!string.IsNullOrEmpty(vehicleImageData?.Base64))
                {
                    picVehicleImageIn.ShowImage(ImageHelper.Base64ToImage(vehicleImageData?.Base64 ?? "") ?? defaultImg);
                }
                else
                {
        ...
        private async Task<bool> BtnExportExcel_Click(object? sender)
        {
            ...
            try
            {
                var report = await ApiServer.ReportingService.Alarm.GetDataAsync(keyword, startTime, endTime, identityGroupId, laneId, user, customerCollectionId, false, cbAlarmMode.SelectedIndex - 1,
        ...
        private async Task<bool> GetReportData(int pageIndex)
        {
            ...
            var report = await ApiServer.ReportingService.Alarm.GetDataAsync(keyword, startTime, endTime, identityGroupId, laneId,
                                                                             user, customerCollectionId, true, cbAlarmMode.SelectedIndex - 1, pageIndex, Filter.PAGE_SIZE);
            if (report == null)
            {
                MessageBox.Show("Không tải được thông tin cảnh báo. Vui lòng thử lại");
                return false;
            }
        ...
        private void DisplayData(List<AbnormalEvent> report)
        {
            try
            {
                dgvData1.Rows.Clear();
                ...
                foreach (var data in report)
                {
                    ...
                    AccessKey? registerVehicle = data.AccessKey?.GetVehicleInfo();
                    CustomerDto? customer = registerVehicle?.Customer;
                    row.Cells[dgvData1.Columns[colEventId.Name].Index].Value = data.Id;
                    row.Cells[dgvData1.Columns[colFileKeys.Name].Index].Value = data.Images == null ?
                                                                                "[]" :
                                                                                Newtonsoft.Json.JsonConvert.SerializeObject(data.Images);
                    ...
                    row.Cells[dgvData1.Columns[colPlateIn.Name].Index].Value = data.PlateNumber;
                    row.Cells[dgvData1.Columns[colTime.Name].Index].Value = data.AlarmTime.ToVNTime();
                    row.Cells[dgvData1.Columns[colNote.Name].Index].Value = data.Note;
                    row.Cells[dgvData1.Columns[colAccessKeyCollection.Name].Index].Value = data.AccessKey?.Collection?.Name ?? "";
                    row.Cells[dgvData1.Columns[colUser.Name].Index].Value = data.CreatedBy;
                    row.Cells[dgvData1.Columns[colLaneInName.Name].Index].Value = data.Device?.Name ?? "";
                    row.Cells[dgvData1.Columns[colAccessKeyName.Name].Index].Value = data.AccessKey?.Name ?? "";
                    row.Cells[dgvData1.Columns[colAccessKeyCode.Name].Index].Value = data.AccessKey?.Code ?? "";
                    ...
                    row.Cells[dgvData1.Columns[colType.Name].Index].Value = AlarmCodeExtension.ToString((EmAlarmCode)data.Code);
        ...
        private void DisplayExportData(List<AbnormalEvent> report, DataGridView dgv)
        {
            try
            {
        ...
        public void Translate()
        {
            ...
            colType.HeaderText = KZUIStyles.CurrentResources.colType;
        }
        private void InitUI()
        {
            try
            {
                ...
                cbIdentityGroupType.DisplayMember = cbLane.DisplayMember = cbUser.DisplayMember = cbCustomerGroup.DisplayMember = "Name";
                cbIdentityGroupType.ValueMember = cbLane.ValueMember = cbUser.ValueMember = cbCustomerGroup.ValueMember = "Value";
                cbIdentityGroupType.MaxDropDownItems = cbLane.MaxDropDownItems = cbUser.MaxDropDownItems = cbCustomerGroup.MaxDropDownItems = 12;
