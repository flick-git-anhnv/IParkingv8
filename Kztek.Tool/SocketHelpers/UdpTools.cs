...
namespace Kztek.Tool
{
    public class UdpTools
    {
        ...
        public static async Task<string> ReceiveSocketResponseAsync(string deviceId, string deviceName, string comport, int baudrate, string cmd, bool isRetry)
        {
            //SystemUtils.logger.SaveDeviceLog(new DeviceLog()
            //{
            //    DeviceId = deviceId,
            //    DeviceName = $"{deviceName} - {comport} - {baudrate}",
            //    Cmd = cmd,
            ...
            {
                return UdpTools.ExecuteCommand_Ascii(comport, baudrate, cmd, 500).Replace("0x02", "").Replace("0x03", "").Replace("\u0002", "").Replace("\u0003", "");
            });
            if (string.IsNullOrEmpty(response) && isRetry)
            {
                await Task.Delay(200);
                return await ReceiveSocketResponseAsync(deviceId, deviceName, comport, baudrate, cmd, false);
            }

            if (!response.Contains("NotEvent", StringComparison.CurrentCultureIgnoreCase))
            {
                SystemUtils.logger.SaveDeviceLog(new DeviceLog()
                {
                    DeviceId = deviceId,
