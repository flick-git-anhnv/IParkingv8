...
namespace IParkingv8.API.Implementation.v8.DataService
{
    public class AccessKeyAPI(IAuth auth) : IAccessKey
    {
                                                                        ...
                                                                        [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = await ApiHelper.GeneralJsonAPIAsync(apiUrl, data, headers, null, timeOut, Method.Post, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.Response.Contains("\"errorCode\":\"ERROR.ENTITY.VALIDATION.FIELD_DUPLICATED\""))
            {
                var response = await GetByCodeAsync(accessKey.Code, timeOut, accessKeyType: accessKey.Type, callerName, lineNumber, filePath, isSaveLog);
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success", response), callerName, lineNumber, filePath, isSaveLog);
            ...
            if (apiResponse.IsSuccess)
            {
                try
                {
                    ...
                    {
                        SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error: " + apiResponse.Response, "", ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    ...
                    return await CreateAsync(accessKey, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                                                                        ...
                                                                        [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, data, headers, null, timeOut, Method.Post, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.Response.Contains("\"errorCode\":\"ERROR.ENTITY.VALIDATION.FIELD_DUPLICATED\""))
            {
                var response = GetByCode(accessKey.Code, timeOut, accessKeyType: accessKey.Type, callerName, lineNumber, filePath, isSaveLog);
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success", response), callerName, lineNumber, filePath, isSaveLog);
                return Tuple.Create(response?.Item1, apiResponse.Response);
            }
            if (apiResponse.IsSuccess)
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return Create(accessKey, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                         ...
                                                         [CallerFilePath] string filePath = "", bool isSaveLog = true, int pageIndex = 0, int pageSize = Filter.PAGE_SIZE)
        {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await GetAllAsync(timeOut, callerName, lineNumber, filePath, isSaveLog, pageIndex, pageSize);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                        ...
                                                        [CallerFilePath] string filePath = "", bool isSaveLog = true, int pageIndex = 0, int pageSize = Filter.PAGE_SIZE)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, filter, headers, null, timeOut, Method.Post, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                var baseResponse = NewtonSoftHelper<BaseReport<AccessKey>>.GetBaseResponse(apiResponse.Response);
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success"), callerName, lineNumber, filePath, isSaveLog);
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return GetAll(timeOut, callerName, lineNumber, filePath, isSaveLog, pageIndex, pageSize);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                                      ...
                                                                      [CallerFilePath] string filePath = "", bool isSaveLog = true, int pageIndex = 0, int pageSize = Filter.PAGE_SIZE)
        {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await GetByConditionAsync(keyword, collectionId, status, timeOut, callerName, lineNumber, filePath, isSaveLog, pageIndex, pageSize);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
         ...
         [
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return GetByCondition(keyword, timeOut, callerName, lineNumber, filePath, isSaveLog, pageIndex, pageSize);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                       ...
                                                       [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            if (apiResponse.IsSuccess)
            {
                try
                {
                    ...
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success", accessKey), callerName, lineNumber, filePath, isSaveLog);
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await GetByCodeAsync(code, timeOut, accessKeyType, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                       ...
                                                       [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, null, headers, parameters, timeOut, Method.Get, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                try
                {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return GetByCode(code, timeOut, accessKeyType, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                                           ...
                                                                           [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = await ApiHelper.GeneralJsonAPIAsync(apiUrl, null, headers, null, timeOut, Method.Get, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                try
                {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await GetByIdAsync(id, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                                           ...
                                                                           [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, null, headers, null, timeOut, Method.Get, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                try
                {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return await GetById(id, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                 ...
                                                 [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = await ApiHelper.GeneralJsonAPIAsync(apiUrl, body, headers, null, timeOut, Method.Patch, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success"), callerName, lineNumber, filePath, isSaveLog);
                return true;
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await UpdateCollectionAsync(accessKeyId, collectionId, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                               ...
                                               [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, body, headers, null, timeOut, Method.Patch, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success"), callerName, lineNumber, filePath, isSaveLog);
                return true;
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return UpdateCollection(accessKeyId, collectionId, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                                 ...
                                                 [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await DeleteByIdAsync(id, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
                                             ...
                                             [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            var apiResponse = ApiHelper.GeneralJsonAPI(apiUrl, null, headers, null, timeOut, Method.Delete, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Success"), callerName, lineNumber, filePath, isSaveLog);
                return true;
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    Auth.Login();
                    return DeleteById(id, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
