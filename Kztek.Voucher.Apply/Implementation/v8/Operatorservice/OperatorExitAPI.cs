...
namespace IParkingv8.API.Implementation.v8.Operatorservice
{
    public class OperatorExitAPI(IAuth auth) : IOperatorExit
    {
        ...
        public async Task<Tuple<ExitData?, BaseErrorData?>?> CreateAsync(string eventId, string laneId, string accessKeyId, string plateNumber, int timeOut = 10000, [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
        ...
        public async Task<bool> UpdateNoteAsync(string eventId, string note, int timeOut = 10000, [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await UpdateNoteAsync(eventId, note, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
        ...
        public async Task<ExitData?> ChangeCollectionAsync(string eventOutId, AccessKey newAccessKey, int timeOut = 10000, [CallerMemberName] string callerName = "", [CallerLineNumber] int lineNumber = 0, [CallerFilePath] string filePath = "", bool isSaveLog = true)
        {
            string server = Auth.ServerConfig.ApiUrl.StandardlizeServerName();
            string apiUrl = $"{server}{GetInitRoute(EmApiObjectType.EXITS)}/{eventOutId}";
            string baseLog = $"Update Exit {eventOutId} AccessKeyId {newAccessKey.Id}";
            SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess(baseLog), callerName, lineNumber, filePath, isSaveLog);
            Dictionary<string, string> headers = new()
            {
                { "Authorization","Bearer " + Auth.ApiAuthResult.AccessToken  }
            ...
            var body = new
            {
                accessKeyId = newAccessKey.Id,
            };
#if DEBUG
            string a = Newtonsoft.Json.JsonConvert.SerializeObject(body);
#endif
            var apiResponse = await ApiHelper.GeneralJsonAPIAsync(apiUrl, body, headers, null, timeOut, Method.Put, isSaveLog, callerName, lineNumber, filePath);
            if (apiResponse.IsSuccess)
            {
                try
                {
            ...
            else
            {
                if (apiResponse.ApiStatusCode == 401)
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error", "Un Auth", ActionType: EmSystemActionType.WARNING), callerName, lineNumber, filePath, isSaveLog);
                    await Auth.LoginAsync();
                    return await ChangeCollectionAsync(eventOutId, newAccessKey, timeOut, callerName, lineNumber, filePath, isSaveLog);
                }
                else
                {
                    SystemUtils.logger.SaveSystemLog(SystemLog.CreateApplicationProccess($"{baseLog} Error Code", apiResponse.ApiStatusCode, ActionType: EmSystemActionType.ERROR), callerName, lineNumber, filePath, isSaveLog);
