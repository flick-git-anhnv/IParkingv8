...
namespace iParkingv5.Controller.KztekDevices.MT166_CardDispenser
{
    ...
    public class MT166_CardDispenserv8_2 : BaseDispenserDevice, ICardDispenser
    {
        ...
        public async void WorkerThread()
        {
            while (stopEvent != null)
            {
                ...
                try
                {
                    ...
                    this.ControllerInfo.IsConnect = !string.IsNullOrEmpty(response) &&
                                                    !response.Contains("exception", StringComparison.CurrentCultureIgnoreCase);

                    // Fixme
                    //response = "GetEvent?/LenCard=0/Card=0/Input=14/ArrayInput=000000000000/Com=NotCom/StateCardDispenserCom1=90/StateCardDispenserCom2=50/StateWork=Working/ArrayInputCur=000000000000\r\n";
                    //ControllerInfo.IsConnect = true;

                    if (this.ControllerInfo.IsConnect == false)
                    {
                    ...
                    if (string.IsNullOrEmpty(response) || !response.Contains("GetEvent?/"))
                    {
                        continue;
                    }
                    ...
                    if (!response.Contains("NotEvent"))
                    {
                        ...
                        bool isCardEvent = _eventType == MT166V8EventType.Reader1 || _eventType == MT166V8EventType.Reader2 ||
                                           ...
                                           _eventType == MT166V8EventType.BTN1_ABNORMAL || _eventType == MT166V8EventType.BTN2_ABNORMAL ||
                                           _eventType == MT166V8EventType.KET_THE ||
                        ...
                        bool isExitEvent = _eventType == MT166V8EventType.Open;

                        bool isCardBeTaken = _eventType == MT166V8EventType.CardbeTaken;
                        if (isCardEvent)
                        {
                        ...
                        {
                            await CallExitEvent(this.ControllerInfo, "1");
                        }
                        else if (isCardBeTaken)
                        {
                            await CallCardBeTakenEvent(this.ControllerInfo, "1");
                        }
                        else
                        {
                            await DeleteCardEvent();
        ...
        private async Task<string> GetEvent()
        {
            try
            {
                ...
                if (!await NetWorkTools.IsPingSuccessAsync(comport, 100))
                {
        ...
        private async Task CallCardEvent(Bdk controller, Dictionary<string, string> map, MT166V8EventType eventType)
        {
            ...
            if (eventType == MT166V8EventType.Reader1 ||
                eventType == MT166V8EventType.Reader2)
            {
                e.ReaderIndex = e.ReaderIndex == 3 ? 1 : (e.ReaderIndex == 4 ? 2 : 3);
            }
            if ((eventType == MT166V8EventType.Button1) || (eventType == MT166V8EventType.Button2))
            {
        ...
        private async Task CallExitEvent(Bdk controller, string doorNo)
        {
        ...
        private async Task CallCardBeTakenEvent(Bdk controller, string doorNo)
        {
        ...
        public async Task<bool> SetAudio(int relayIndex)
        {
            try
            {
                ...
                string response = await UdpTools.ReceiveSocketResponseAsync(this.ControllerInfo.Id, this.ControllerInfo.Name, comport, baudrate, setAudioCmd, true);
                if (UdpTools.IsSuccess(response, "OK"))
                {
                    return true;
                }
...
//{
//    isTest = false;
//    response = "GetEvent?/LenCard=4/Card=31C0C7C7/Input=34/ArrayInput=000000000100/Com=Com1/StateCardDispenserCom1=NotConnect/StateCardDispenserCom2=NotConnect/StateWork=Working/ArrayInputCur=000000000100/";
//}