...
namespace iParkingv5.Controller.ZktecoDevices
{
    public class QR500
    {  // Serial
        ...
        public bool Connect(string comport, int baudrate, int communicationType)
        {
            try
            {
                ...
                //MessageBox.Show("Kết nối " + IsConnect);
                if (IsConnect)
                {
                    SettingUp();
                }
        ...
        public void SendGetEventCmd()
        {
            try
            {
                ...
                byte[] command = { 0xAA, 0xFF, 0x97, 0x01, 0x00, 0x22, 0xE9, 0xB9 };
                serialPort.Write(command, 0, command.Length);
                Thread.Sleep(200);
                ...
                else if (buffer.Length > 15)
                {
                    ...
                    string voucherData = Encoding.ASCII.GetString(displayVoucher.ToArray());
                    CardEventArgs ce = new();
