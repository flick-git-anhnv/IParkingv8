    ...
    public class HSR900
    {
        ...
        public bool Connect(string _ComPort, int _BaudRate, int _CommunicationType)
        {
            ...
            catch (Exception ex)
            {
                ...
                //LogHelperv2.SaveErrorLog("Connect: " + ex.Message);
            }
            IsConnect = false;
            return false;
        }
        ...
        //{
        //    LogHelperv2.SaveErrorLog(e.EventType.ToString());
        //}
        ...
        //    catch (Exception exp)
        //    {
        //        LogHelperv2.SaveErrorLog("Error while writing to logfile: " + exp.Message);
        ...
        private string ExecuteCommand(byte[] buffer, string command, ref string viewraw, ref string[] message)
        {
            ...
            catch (Exception ex)
            {
               ...
               ///LogHelperv2.SaveErrorLog("Thuc hien lenh den thiet bi (pc <-> host)\n" + ex.Message);
            }
            return "";
        }
        ...
        public void WorkerThread()
        {
            while (!stopEvent.WaitOne(0, true))
            {
                                ...
                                if (MESSID != "" && message.Length == 20 && message[0] == "02" && message[19] == "03")
                                {
                                    ...
                                    if (message[1] == "4E" && str != "30303030303030303030303030303030" && CardEvent != null)
                                    {
                                        CardEventArgs ex = new CardEventArgs
                                        {
                                            ...
                                            CardNumber = MESSID.Substring(2, 16),
                ...
                catch (Exception ex)
                {
                    ...
                    //LogHelperv2.SaveErrorLog("WorkerThread\n" + lineID.ToString() + "\n" + ex.Message);
                }
            }
        }
                                ...
                                if (ExecuteCommand(bytes, "", ref viewraw, ref message) != "" && message[2] == "06")
                                {
                                    for (int i = 4; i <= blacklistcount*4;)
                                    {
                                        BlackList blacklist = new BlackList
                                        {
                                            ...
                                            CardNumber = message[i + 3] + message[i + 2] + message[i + 1] + message[i]
                                        };
