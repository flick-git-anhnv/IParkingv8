    ...
    public class PP6750
    {
                ...
                try
                {
                    if (serialPort.IsOpen)
                    {
                        foreach (Futech.Objects.Controller controller in controllers)
                        {
                            ...
                            if (message != null)
                            {
                                if (message.Length == 48 && CardEvent != null)
                                {
                                    CardEventArgs e = new CardEventArgs();
                                    e.LineID = controller.LineID;
                                    e.ControllerAddress = controller.Address;
                                    e.CardNumber = GetValue(message[13]) + GetValue(message[14]) + GetValue(message[15]) + GetValue(message[16]) + GetValue(message[17]) + GetValue(message[18]) + GetValue(message[19]) + GetValue(message[20]);
                                    e.Date = "20" + GetValue(message[25]) + GetValue(message[26]) + "/" + GetValue(message[27]) + GetValue(message[28]) + "/" + GetValue(message[29]) + GetValue(message[30]);
                                    e.Time = GetValue(message[33]) + GetValue(message[34]) + ":" + GetValue(message[35]) + GetValue(message[36]);
                                    e.ReaderIndex = 1;
                                    
                                    if (GetValue(message[5]) == "8")
                                    {
                                        if (GetValue(message[6]) == "1")
                                            e.EventStatus = "PIN Error";
                                        else if (GetValue(message[6]) == "2")
                                            e.EventStatus = "Access Denied";
                                        else if (GetValue(message[6]) == "3")
                                            e.EventStatus = "Timezone Error";
                                        else if (GetValue(message[6]) == "9")
        ...
        public bool DownloadCard(string cardNumber, string password, int timezoneID)
        {
            if (serialPort.IsOpen)
            {
                ...
                string card7 = cardNumber.Substring(6, 1);
                string card8 = cardNumber.Substring(7, 1);
                //
                ...
                byte[] bytes = new byte[] { 0x12, 0x50, 0x4D, GetHighByte(address), GetLowByte(address), GetByte(card1), GetByte(card2), GetByte(card3), GetByte(card4), GetByte(card5), GetByte(card6), GetByte(card7), GetByte(card8), GetHighByte(temp), GetLowByte(temp), 0x31, 0x30, GetByte(pass1), GetByte(pass2), GetByte(pass3), GetByte(pass4), 0x47, 0x0D, 0x0A };
                // request to reader
                Request(bytes);
                // wait to reply from reader
        ...
        public bool DeleteCard(string cardNumber, string password)
        {
            if (serialPort.IsOpen)
            {
                ...
                string pass4 = password.Substring(3, 1);
                //
                byte[] bytes = new byte[] { 0x12, 0x50, 0x4D, GetHighByte(address), GetLowByte(address), GetByte(card1), GetByte(card2), GetByte(card3), GetByte(card4), GetByte(card5), GetByte(card6), GetByte(card7), GetByte(card8), 0x30, 0x30, 0x30, 0x30, GetByte(pass1), GetByte(pass2), GetByte(pass3), GetByte(pass4), 0x47, 0x0D, 0x0A };
                // request to reader
                Request(bytes);
                // wait to reply from reader
                Thread.Sleep(delaytime * 2);
