...
namespace Futech.Objects
{
    ...
    public class GraphControl : UserControl
    {
        ...
        ArrayList PointsStart;
        ArrayList PointsFinish;
        ...
        public GraphControl()
        {
            ...
            GRAPHSTRUCT = new GraphStruct(24, 8, 60, 1, 60, 1);
            Points = new ArrayList();
            PointsStart = new ArrayList();
            PointsFinish = new ArrayList();
        ...
        public void AddLine(int startX, int finishX, int startY, int finishY)
        {
            PointsStart.Add(new Point(startX, finishX));
            PointsFinish.Add(new Point(startY, finishY));
        }
        ...
        void DrawPoints(Graphics g)
        {
            ...
            for (int i = 0; i <= GRAPHSTRUCT.PointsOnXAxis; i++)
            {
                ...
                float val1 = val / 60;
                g.DrawString(val1.ToString(), f, b, xAxisX - 5, xAxisY + 3);
        ...
        void DrawLineGraph(Graphics g)
        {
            ...
            Pen p1 = new Pen(axesColor);
            p1.DashStyle = DashStyle.Dot;
            p.Width = 5;
            FloatPoint prev, current;
            for (int i = 0; i < PointsStart.Count; i++)
            {
                Point ptStart = (Point)PointsStart[i];
                Point ptFinish = (Point)PointsFinish[i];
                prev = FindLocationOnGraph(ptStart);
                current = FindLocationOnGraph(ptFinish);
                g.DrawLine(p, prev.X, prev.Y, current.X, current.Y);
                g.DrawLine(p1, prev.X, origin.Y, prev.X, prev.Y);
                g.DrawLine(p1, current.X, origin.Y, current.X, current.Y);
                /*
                if (bShowPoints)
                {
                    Brush b = new SolidBrush(textColor);
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            //drawAxes is called twice because the axes get overwwiten sometimes by other lines
            DrawAxes(pe.Graphics);
        }
