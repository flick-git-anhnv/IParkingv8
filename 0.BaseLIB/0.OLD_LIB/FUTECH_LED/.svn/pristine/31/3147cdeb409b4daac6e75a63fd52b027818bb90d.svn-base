...
namespace ModbusTCP
{
    ...
    public  class Device
    {
        ...
        private static ushort _timeout = 1000;
        ...
        public Device(string ip, ushort port, ushort timeout)
        {
            _timeout = timeout;
            connect(ip, port);
        }
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    Console.WriteLine("qua nguy hiem");
                    //int result = 0;
