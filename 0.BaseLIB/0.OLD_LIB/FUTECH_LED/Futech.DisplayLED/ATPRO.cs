...
{
    internal class ATPRO : IDisplayLED
    {
        ...
        DemoModbusTCPIP.Snap7 myGetSet = new DemoModbusTCPIP.Snap7();
        Device d;
        Byte[] WriteHolding = new byte[4];// kiêu dword là 2 word 4 byte(1 word 2 byte như project trước)
        ...
        public bool Connect(string comport, int baudrate)
        {
            try
            {
                ...
                else
                {
                    ...
                    d = new Device(comport, Convert.ToUInt16(baudrate), 1000)
                    {
                        DeviceIp = comport,
                        port = baudrate.ToString(),
                        uid = this.address.ToString()
                    };
                    if (d.connected == false)
                    {
                        int k = 0;
                        while (k < 3 && d.connected == false)
                        {
                        ...
                        if (d.connected == false)
                        {
                            // label7.Text = "kết nối đến server thất bại";
                            return false;
                        }
                    ...
                    {
                        //label7.Text = "kết nối đến server thành công";
        ...
        public void SendToLED(string dtimein, string dtimout, string plate, string money, int cardtype, string cardstate)
        {
            try
            {
                ...
                {
                    myGetSet.SetDWord(WriteHolding, 0, Convert.ToUInt32(money.Replace(",", "").Replace(".", "")));// set vào mãng 4 byte cho dword
                    for (int i = 0; i < 3; i++)
                    {
                        d.WriteMultipleRegister(Convert.ToUInt16(d.uid), Convert.ToByte(d.uid), Convert.ToUInt16(Convert.ToDouble("40001") - 40001), WriteHolding, ref result);
                    }
