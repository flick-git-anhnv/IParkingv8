...
{
    internal class PGS_LED : IDisplayLED
    {
        ...
        public void SendToLED(string dtimein, string dtimout, string plate, string money, int cardtype, string cardstate)
        {
            try
            {
                ...
                if (CommunicationType == 0)
                {
                    ...
                    byte add1 = Convert.ToByte(ByteUI.Asc(address.ToString("000")[0]));
                    byte add2 = Convert.ToByte(ByteUI.Asc(address.ToString("000")[1]));
                    byte add3 = Convert.ToByte(ByteUI.Asc(address.ToString("000")[2]));

                    int parkingspace = DateTime.Now.Hour * 60 + DateTime.Now.Minute;


                    byte Num1, Num2, Num3, Num4;
                    Num1 = Convert.ToByte(ByteUI.Asc(parkingspace.ToString("0000").Substring(0, 1)));
                    Num2 = Convert.ToByte(ByteUI.Asc(parkingspace.ToString("0000").Substring(1, 1)));
                    Num3 = Convert.ToByte(ByteUI.Asc(parkingspace.ToString("0000").Substring(2, 1)));
                    Num4 = Convert.ToByte(ByteUI.Asc(parkingspace.ToString("0000").Substring(3, 1)));
                    byte[] bytes = new byte[] { 0x2F, 0x50, add1, add2, 0x30, 0x30, Num1, Num2, Num3, Num4, 0x3E, 0x00 };
                    bytes[11] = ByteUI.CRC(bytes, 12, 1);
                    if (port.IsOpen)
                    {
                ...
                else if (CommunicationType == 1)
                {
                    ...
                    //temp[0] = 0x01;//header
                    //temp[1] = 0x30;//Convert.ToByte(ByteUI.Asc(address.ToString("000")[0]));//add1
                    //temp[2] = 0x30;//Convert.ToByte(ByteUI.Asc(address.ToString("000")[1]));//add2
                    //temp[3] = 0x31;//;Convert.ToByte(ByteUI.Asc(address.ToString("000")[2]));//add3
                    //temp[4] = 0x31;
