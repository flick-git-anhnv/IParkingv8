...
namespace TcpipIntface.Code.Client
{
    ...
    public class AcsTcpClass
    {
     ...
     //   public event TOnCardStatusHandler OnCardStatusHandler;        //声明事件
        ...
        // =====================================================================================================
        public void EventHandler(TcpipIntface.Code.Client.RAcsEvent Event, out byte relay, out Boolean OpenDoor, out Boolean Ack)
        {
            OpenDoor = false;
            Ack = true;
            relay = Event.Reader;
        ...
        public void EventChinaHandler(TcpipIntface.Code.Client.RChinaCard Event, out Boolean OpenDoor, out Boolean Ack)
        {
            OpenDoor = false;
            Ack = true;
        ...
        public void DoOnDataDebug(byte rt, byte[] buff, int len)
        {
            ...
            if (OnDataDebug != null)
            {
               byte[] returnBytes = new byte[len];
                ...
                if (rt == 0)
                { str = string.Concat("take over ", str); }
                else
