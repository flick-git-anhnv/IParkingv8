...
namespace IParkingv8.Cash.Controllers
{
    public class CValidator
    {
                ...
                switch (m_cmd.ResponseData[i])
                {
                    ...
                    case CCommands.SSP_POLL_READ_NOTE:
                        if (m_cmd.ResponseData[i + 1] > 0)
                        {
                            ...
                            long money = long.Parse(CHelpers.FormatToCurrency(noteVal));
                            return money;
                        }
                        else
                ...
                switch (m_cmd.ResponseData[i])
                {
                    ...
                    case CCommands.SSP_POLL_READ_NOTE:
                        if (m_cmd.ResponseData[i + 1] > 0)
                        {
                            ...
                            log.AppendText("Note in escrow, amount: " + CHelpers.FormatToCurrency(noteVal) + " " + GetChannelCurrency(m_cmd.ResponseData[i + 1]) + "\r\n");
                            m_HoldCount = m_HoldNumber;
                            moneyTest = long.Parse(CHelpers.FormatToCurrency(noteVal));
                        }
                        else
                            log.AppendText("Reading note...\r\n");
                        i++;
                    ...
                    case CCommands.SSP_POLL_CREDIT_NOTE:
                        ...
                        csg.MoneyValue = long.Parse(CHelpers.FormatToCurrency(noteVal));
                        csg.MoneyPaid += csg.MoneyValue;

                        log.AppendText("Credit " + textMoney + "\r\n");
                        //m_NumStackedNotes++;
        ...
        // Reset lại máy thanh tóan, thoát khỏi vòng loop
        ...
        /// <summary>
        /// Bỏ log test các chức năng khác 
        /// </summary>
        /// <returns></returns>
        public async Task<bool> SendCommand2(TextBox log)
        {
            // Backup data and length in case we need to retry
            byte[] backup = new byte[255];
            m_cmd.CommandData.CopyTo(backup, 0);
