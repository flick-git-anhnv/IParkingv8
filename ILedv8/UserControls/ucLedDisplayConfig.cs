...
{
    public partial class ucLedDisplayConfig : UserControl
    {
        public ucLedDisplayConfig()
        {
            InitializeComponent();
        ...
        private void BtnConfirm_Click(object? sender, EventArgs e)
        {
            ...
            foreach (ucLedLineColorConfig item in panelConfig.Controls)
            {
                var config = item.GetConfig();
                NewtonSoftHelper<LedColorConfig>.SaveConfig(config, IparkingingPathManagement.ledColorDisplayConfig(item.led.comport, item.line));
            }
        ...
        private void CbLeds_SelectedIndexChanged(object? sender, EventArgs e)
        {
            ...
            {
                ucLedLineColorConfig uc = new ucLedLineColorConfig(led, i + 1);
